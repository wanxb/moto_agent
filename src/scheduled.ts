...
import { getActiveReminders, getLatestOdometer, markReminderDone } from './database';
...
export function formatReminder(r: DueReminder): string {
  ...
  if (r.mode === 'mileage') {
    ...
    return `🔔 保养提醒${tag}\n该处理「${r.type}」了：当前 ${cur} ≥ 提醒里程 ${trig} km`;
  }
...
export async function runScheduled(
  ...
  for (const r of due) {
    ...
    try {
      await send(target, formatReminder(r));
