...
import { AnthropicLLM } from './infra/anthropic-llm';
...
export function bootstrap(env: Env): App {
  ...
  const llm = new FallbackLLM(
    new DeepSeekLLM(env.DEEPSEEK_API_KEY),
    new AnthropicLLM(env.ANTHROPIC_API_KEY),
  );

  const tools = registry.toOpenAI();
  const agent: AgentRunner = (messages: Message[], db: D1Database) =>
    runAgentLoop(messages, llm, tools, registry, db);
