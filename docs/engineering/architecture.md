...
| **国际化** | `src/i18n/`（📋 Planned） + `src/prompts.ts` | `t(key, lang, ...args)` 翻译函数；KV 存 `lang:{chatId}` 偏好；系统提示中英双语；`ToolRegistry.toOpenAI(lang)` 按语言选工具描述 | 业务逻辑 · [ADR-0008](adr/0008-i18n-bilingual.md) |
...
| 国际化 | 📋 Planned：`src/i18n/` + 系统提示/工具描述/输出双语 | [ADR-0008](adr/0008-i18n-bilingual.md) |
| Web Dashboard | 📋 Planned：Worker 内嵌 HTML + Chart.js + REST API，只读可视化 | [ADR-0009](adr/0009-phase3-dashboard.md) |
