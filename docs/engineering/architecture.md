...
单 Cloudflare Worker，无状态。两类入口：**Telegram webhook**（Bot）与 **HTTP**（PWA 前端 + REST/auth/chat API）。所有持久状态外置到 D1（业务数据 + 用户）与 KV（会话历史、session、magic/bind link）。

> **spec 016 起多用户**：Worker 既是 Bot 后端，也经 `[assets]` 托管 `web/` 的 Svelte SPA（对话/登录/设置/仪表盘），并提供 `/auth`、`/chat`、`/api/v1` 端点。Telegram 开放自助（任何人可用、自动建号），PWA 走邮箱 Magic Link 认证，数据按 `user_id` 隔离。详见 [ADR-0010](adr/0010-frontend-svelte-spa-static-assets.md) 与 [spec 016](../specs/016-multi-user-pwa/)。
...
| **数据访问** | `src/database.ts` | 纯 SQL 的 D1 CRUD（含用户 CRUD + `user_id` 隔离） | 业务计算 |
| **认证** | `src/routes/auth-handler.ts` + `services/{mail,session}.ts` | Magic Link 登录、登出、账号绑定（链接式）、session 签发/校验/滑动续期、Resend 发信（spec 016） | 业务逻辑 |
| **PWA 对话/数据 API** | `src/routes/chat-api.ts` + `api.ts` | `/chat/api`(对话+历史)、`/chat/voice`(语音)、`/api/v1/*`(仪表盘数据+`/me`)；session 鉴权 + `user_id` 过滤 | — |
| **PWA 前端** | `web/`(Vite+Svelte SPA) | 对话/登录/设置/仪表盘；经 `[assets]` 托管（spec 016 / [ADR-0010](adr/0010-frontend-svelte-spa-static-assets.md)） | 由 Worker 提供，独立构建 |
| **类型** | `src/types.ts` | `Env`、消息/工具/LLM/记录/`User` 类型 | — |
...
4. 开放自助：无白名单（spec 016）；`pipeline.resolveUserId → getOrCreateTelegramUser` 按 `chat_id` 解析/自动建号，得到 `user_id`。
5. `message:text` handler → `bootstrap(env)` → `TelegramAdapter(ctx, env)` → `app.run(adapter, { text })`。
6. `pipeline.ts` 接管执行：语言检测 → 限流 → 解析 `user_id` → `ISessionStore.get(chatId)` 读 KV 历史 → push 当前消息。
...
| 状态种类 | 存哪 | Key/结构 | 生命周期 |
|---------|------|---------|---------|
| 会话历史（TG 多轮上下文） | **KV** `SESSION_KV` | `session:{chatId}` → JSON 消息数组 | 最近 10 条，TTL 1h |
| PWA 对话历史（spec 016） | **KV** `SESSION_KV` | `session:pwa:{user_id}` → JSON 消息数组 | TTL 1h，独立于 TG |
| 登录 session（spec 016） | **KV** `SESSION_KV` | `session_token:{token}` → `{user_id,email,...}` | 30 天滑动续期 |
| Magic/Bind link token（spec 016） | **KV** `SESSION_KV` | `magic_link:{t}` / `bind_link:{t}` | 15 / 10 分钟一次性 |
| 限流计数 | **KV** `SESSION_KV` | `rate:*` / `auth:send:*` | 滑动窗口 |
| 业务数据 + 用户 | **D1** `DB` | 关系表（含 `users`、各表 `user_id`） | 永久 |
