...
| **P1** | 维修保养记录 | 高（高频真实需求） | 中 | 多车（保养绑定到车）✔️ | [`../specs/002-maintenance/`](../specs/002-maintenance/) 🚧 开发中 |
| **P1** | 定时提醒（保养/保险） | 高 | 中（引入 Cron Triggers） | 维保记录 | [`../specs/backlog.md`](../specs/backlog.md) |
| **P2** | 记录纠错/删除 | 中（数据质量） | 低 | 无 | [`../specs/backlog.md`](../specs/backlog.md) |
...
- **M2.2** 维保记录上线：可记录并查询保养历史。🚧 开发中（spec 002）。
- **M2.3** 定时提醒上线：Cron Triggers 跑通，里程/日期阈值触发推送。
- **M2.4** 纠错能力上线：可修改/删除最近记录。
